...
/**
 * glTFオブジェクトレンダラーの基底クラス Genericsを使用して様々なオブジェクト型に対応
 *
...
public abstract class GltfObjectRenderer<T> {
     ...
     */
    public abstract RenderingContext.Builder createRenderContext(
            T object, ModelTransformationMode mode, @Nullable LivingEntity entity);

    /** レンダーレイヤーの決定 (継承先で実装) 各オブジェクトタイプ固有のテクスチャマッピングロジックが必要 */
    protected abstract RenderLayer getRenderLayer(
            RenderingContext context, ProcessedMesh mesh, ModelMetadata metadata);

    protected abstract List<String> getHideKeys(
    ...
    public void render(
        ...
        try {
            ...
            contextBuilder.light(light).overlay(overlay).entity(entity).tickDelta(tickDelta);
    ...
    private boolean canRenderGltfModel(
            @Nullable RenderingContext context, @Nullable ProcessedGltfModel model) {
