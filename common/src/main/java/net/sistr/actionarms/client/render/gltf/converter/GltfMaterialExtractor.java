...
/** glTFマテリアルからProcessedMaterialへの変換処理 jglTFのMaterialModelから必要な情報を抽出してProcessedMaterialを生成 */
public class GltfMaterialExtractor {
            ...
            for (int i = 0; i < materialModels.size(); i++) {
                ...
                try {
                    ...
                    ActionArms.LOGGER.debug(
                            "Extracted material {}: {}", i, processedMaterial.getDebugInfo());
     ...
     * @return 変換済みマテリアル
     */
    private ProcessedMaterial extractSingleMaterial(
    ...
    private ProcessedMaterial createFallbackMaterial(int materialIndex) {
        return ProcessedMaterial.builder().name("FallbackMaterial_" + materialIndex).build();
    }
