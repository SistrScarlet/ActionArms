...
/** 変換済みスキンデータ管理recordクラス（不変設計） ボーン階層とその関係性を管理 */
public record ProcessedSkin(String name, List<ProcessedBone> bones, List<ProcessedBone> rootBones) {
  ...
  public Optional<ProcessedBone> findBone(String boneName) {
    return bones.stream().filter(bone -> Objects.equals(bone.name(), boneName)).findFirst();
  }

  /** インデックスでボーンを検索 */
  public Optional<ProcessedBone> findBone(int boneIndex) {
    return bones.stream().filter(bone -> bone.index() == boneIndex).findFirst();
  }
    ...
    for (ProcessedBone bone : bones) {
      ...
      if (hasCircularReference(bone)) {
        throw new IllegalStateException(
            "invalid skin hierarchy: %s has circular reference: %s"
  ...
  public int getMaxDepth() {
    return bones.stream().mapToInt(ProcessedBone::getDepth).max().orElse(0);
  }

  @Override
  public @NotNull String toString() {
    return String.format(
        "ProcessedSkin[%s: %d bones, %d roots, depth=%d]",
  ...
  public static class Builder {
    ...
    public ProcessedSkin build() {
      var skin =
          new ProcessedSkin(name, bones, rootBones.isEmpty() ? findRootBones(bones) : rootBones);
  ...
  public Builder toBuilder() {
    return builder().name(name).addBones(bones).rootBones(rootBones);
  }
}
