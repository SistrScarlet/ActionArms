    ...
    public ProcessedGltfModel convertModel(String name, GltfModel gltfModel, int sceneIndex) {
        ...
        try {
            ...
            ActionArms.LOGGER.info(
                    "Extracted {} materials from model {}", processedMaterials.size(), name);
        ...
        try {
            ...
            ActionArms.LOGGER.info(
                    "Extracted {} animations from model {}", processedAnimations.size(), name);
        } catch (RuntimeException e) {
        ...
        for (NodeModel rootNode : sceneModel.getNodeModels()) {
            ...
            try {
                ...
                for (MeshModel meshModel : rootNode.getMeshModels()) {
                    var meshes =
                            vertexExtractor.extractMeshes(
                                    processedMaterials, processedSkin, meshModel);
