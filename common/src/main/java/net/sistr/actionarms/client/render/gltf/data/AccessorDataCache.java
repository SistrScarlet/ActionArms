...
/** AccessorDataのキャッシュ管理クラス 同じアクセサデータの重複作成を防ぎ、メモリ効率を向上させる */
public class AccessorDataCache {
  private final Map<AccessorSignature, net.sistr.actionarms.client.render.gltf.data.AccessorData>
      cache;
  ...
  /** アクセサデータを取得または作成 キャッシュにある場合は既存のものを返し、ない場合は新規作成してキャッシュに保存 */
  ...
  public net.sistr.actionarms.client.render.gltf.data.AccessorData getOrCreate(
    ...
    if (cached != null) {
      ...
      ActionArms.LOGGER.debug(
          "Cache hit for accessor: {}", customId != null ? customId : "unnamed");
      return cached;
    }

    missCount++;
    ActionArms.LOGGER.debug(
        "Cache miss for accessor: {}, extracting data...", customId != null ? customId : "unnamed");
  ...
  private AccessorSignature createSignature(AccessorModel accessor) {
    ...
    int bufferViewHash =
        accessor.getBufferViewModel() != null
            ? System.identityHashCode(accessor.getBufferViewModel())
            : 0;
  ...
  private float[] extractFloatArray(AccessorModel accessor, int componentCount) {
    ...
    if (!(accessorData instanceof AccessorFloatData floatData)) {
      throw new IllegalArgumentException(
          "Expected float data but got: " + accessorData.getClass().getSimpleName());
    }
  ...
  private int[] extractIntArray(AccessorModel accessor, int componentCount) {
    ...
    if (accessorData instanceof AccessorIntData intData) {
      ...
      throw new IllegalArgumentException(
          "Expected int or short data but got: " + accessorData.getClass().getSimpleName());
    }
  ...
  public record CacheStats(int entryCount, long totalMemoryUsage, int hitCount, int missCount) {
    ...
    public String toString() {
      return String.format(
          "CacheStats[entries=%d, memory=%s, requests=%d, hitRate=%.1f%%]",
