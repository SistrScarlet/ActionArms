...
/** モーフターゲットrecordクラス（AccessorDataベース・不変設計） float[]配列の代わりにAccessorDataを直接保持してメモリ効率を向上 */
public record MorphTarget(
  ...
  /** 位置と法線のみのコンストラクタ（推奨） */
  public MorphTarget(
      String name, @Nullable AccessorData positionDeltas, @Nullable AccessorData normalDeltas) {
  ...
  public Vector3f getPositionDelta(int vertexIndex) {
    ...
    if (!hasPositionDeltas()
        || vertexIndex < 0
        || vertexIndex >= positionDeltas.getElementCount()) {
  ...
  /** 位置差分の生配列を取得（読み取り専用） 注意: パフォーマンス重視の場合のみ使用、通常はgetPositionDelta()を推奨 */
  ...
  public String getDebugInfo() {
    return String.format(
        "MorphTarget[%s: elements=%d, types=%s, memory=%d bytes]",
  ...
  public @NotNull String toString() {
    return String.format(
        "MorphTarget[%s: %d elements, %s]", name, getElementCount(), getDataTypes());
  }
