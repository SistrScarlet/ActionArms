...
/** 変換済みアニメーションチャンネルrecordクラス（不変設計） キーフレーム補間とアニメーション値計算を管理 */
public record ProcessedChannel(
    ...
    public ChannelStats getStats() {
        return new ChannelStats(
                getKeyframeCount(), duration, getValueType(), interpolationMode, isValid());
    }

    /** デバッグ情報を取得 */
    public String getDebugInfo() {
        return String.format(
                "ProcessedChannel[%s.%s: %d keys, %.3fs, %s, %s]",
                targetNode,
                targetPath,
                getKeyframeCount(),
                duration,
