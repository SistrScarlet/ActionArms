...
/** 変換済みマテリアルデータ管理クラス（Record版） glTFのMaterialModelから変換されたマテリアル情報を保持 */
public record ProcessedMaterial(
    ...
    public ProcessedMaterial {
        ...
        if (alphaMode == null) {
            alphaMode = "OPAQUE";
        } else if (!alphaMode.equals("OPAQUE")
                && !alphaMode.equals("MASK")
                && !alphaMode.equals("BLEND")) {
    ...
    public String getDebugInfo() {
        ...
        sb.append(
                String.format(
                        ", factors[base=%.2f,%.2f,%.2f,%.2f, metallic=%.2f, roughness=%.2f]",
                        baseColorFactor[0],
                        baseColorFactor[1],
                        baseColorFactor[2],
                        baseColorFactor[3],
    ...
    public static ProcessedMaterial createDefault() {
        return builder().name("DefaultMaterial").build();
    }
