...
/** glTFメタデータ管理クラス JSONメタデータの読み込みと管理を行う */
public class GltfMetadataManager implements ResourceReloader, AutoCloseable {
        ...
        return CompletableFuture.supplyAsync(
                        () -> {
                            ...
                            var metadataResources =
                                    manager.findResources(
                                            "gltf/models", id -> id.getPath().endsWith(".json"));
                            ...
                            for (var entry : metadataResources.entrySet()) {
                                ...
                                try (var reader =
                                    ...
                                    Identifier metadataKey =
                                            new Identifier(resourceId.getNamespace(), metadataId);

                                    metadata.put(metadataKey, parsedMetadata);
                                    ActionArms.LOGGER.debug(
                                            "Loaded glTF metadata: {} -> {}",
                                            metadataKey,
                                            parsedMetadata.modelPath());
                                } catch (IOException | JsonParseException e) {
                                    ActionArms.LOGGER.error(
                                            "Failed to load glTF metadata: {}", resourceId, e);
                                }
                            }
                ...
                .thenAcceptAsync(
                        metadata -> {
                            ...
                            ActionArms.LOGGER.info(
                                    "Loaded {} glTF metadata entries", metadata.size());
    ...
    /** JSONからModelMetadataをパース 実際のファイル形式に完全対応 */
    private ModelMetadata parseMetadata(Identifier resourceId, JsonObject json) {
        try {
            // 1. "model"フィールドの解析（必須）
            if (!json.has("model")) {
                throw new JsonParseException(
                        "Missing required 'model' field in metadata: " + resourceId);
            }
            ...
            ActionArms.LOGGER.error(
                    "Error parsing metadata for {}: {}", resourceId, e.getMessage());
        ...
        if (textureSettingsObj.has("texture_map")) {
            ...
            for (var entry : textureMapObj.entrySet()) {
                ...
                String resourcePathStr =
                        entry.getValue().getAsString(); // "actionarms:textures/test/skin_alex.png"
