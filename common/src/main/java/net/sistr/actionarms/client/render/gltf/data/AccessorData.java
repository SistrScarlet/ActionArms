...
/** glTFアクセサから抽出されたプリミティブ配列データを保持するクラス 元のAccessorModelは保持せず、必要なデータのみを効率的に管理する */
public class AccessorData {
    ...
    public AccessorData(
            @Nullable String id,
            AccessorDataType dataType,
            int elementCount,
            float[] floatData,
            boolean normalized,
            @Nullable AccessorSignature signature) {
    ...
    private void validateData() {
        ...
        if (dataType.isFloatType() && floatData == null) {
            throw new IllegalArgumentException(
                    "Float data expected but not provided for " + dataType);
        }
        if (dataType.isIntType() && intData == null) {
            throw new IllegalArgumentException(
                    "Int data expected but not provided for " + dataType);
        }
    }
    ...
    public String getDebugInfo() {
        return String.format(
                "AccessorData[id=%s, type=%s, elements=%d, stride=%d, normalized=%s, memory=%d bytes, valid=%s]",
    ...
    /** 生のfloat配列を取得（読み取り専用） 注意: 返される配列は防御的コピーではないため、変更してはいけない */
    ...
    /** 生のint配列を取得（読み取り専用） 注意: 返される配列は防御的コピーではないため、変更してはいけない */
