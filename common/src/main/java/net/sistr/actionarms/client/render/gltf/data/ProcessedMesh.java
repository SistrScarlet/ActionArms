...
/** メッシュデータ管理recordクラス（マテリアル参照対応・不変設計） glTFプリミティブから変換されたメッシュ情報を保持 */
public record ProcessedMesh(
    ...
    for (Map.Entry<String, AccessorData> entry : attributeData.entrySet()) {
      ...
      if (data.getElementCount() != expectedVertexCount) {
        throw new IllegalArgumentException(
            String.format(
                "Attribute %s has %d elements, expected %d for mesh %s",
  ...
  public boolean hasAttribute(String attributeName) {
    return attributeData.containsKey(attributeName) && attributeData.get(attributeName).isValid();
  }
  ...
  /** マテリアルを安全に取得 マテリアルが設定されていない場合はデフォルトマテリアルを返す */
  ...
  public String getDebugInfo() {
    ...
    sb.append(
        String.format(
            "ProcessedMesh[%s: %d vertices, mode=%s, primitive=%d",
  ...
  public @NotNull String toString() {
    return String.format(
        "ProcessedMesh[%s: %d vertices, %s, %s]",
