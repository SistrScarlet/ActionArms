...
/** 変換済みアニメーションrecordクラス（不変設計） チャンネル管理とボーン別アクセス最適化を提供 */
public record ProcessedAnimation(
    ...
    public Optional<ProcessedChannel> findChannel(String targetNode, String targetPath) {
        return channels.stream()
                .filter(
                        channel ->
                                Objects.equals(channel.targetNode(), targetNode)
    ...
    /** 指定されたボーンのチャンネル配列を取得 [0] = translation, [1] = rotation, [2] = scale */
    @Nullable
    ...
    public AnimationStats getStats() {
        return new AnimationStats(
                getChannelCount(), getBoneCount(), duration, getChannelTypeStats(), isValid());
    }
    ...
    public String getDebugInfo() {
        ...
        Map<String, Integer> typeStats = getChannelTypeStats();
        typeStats.forEach((type, count) -> sb.append(String.format("    %s: %d", type, count)));
    ...
    public @NotNull String toString() {
        return String.format(
                "Animation[%s: %.2fs, %d channels, %d bones]",
    ...
    public static class Builder {
        ...
        private static Map<String, ProcessedChannel[]> buildNameByChannelsMap(
                List<ProcessedChannel> channels) {
            ...
            for (ProcessedChannel channel : channels) {
                ...
                if (index != -1) {
                    ProcessedChannel[] targetChannels =
                            mapping.computeIfAbsent(
                                    channel.targetNode(), s -> new ProcessedChannel[3]);
        ...
        public ProcessedAnimation build() {
            ...
            return new ProcessedAnimation(
                    name, channels, nameByChannels, calculateDuration(channels));
        }
    }
    ...
    public Builder toBuilder() {
        return builder().name(name).channels(channels);
    }
}
