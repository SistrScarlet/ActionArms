...
public class GltfMeshRenderer {
  ...
  public void renderMesh(
    ...
    try {
      ...
      for (int vertexIndex : indices) {
        renderVertex(
            mesh, vertexIndex, boneMatrices, morphWeights, matrixStack, vertexConsumer, context);
      }
  ...
  private void computeAnimationData(
    ...
    var animations =
        Arrays.stream(context.animations())
            .map(
                state -> model.getAnimation(state.name()).map(value -> new Animation(value, state)))
  ...
  /** hide_contextsによるボーン非表示化 RenderingContextのhiddenBonesリストに含まれるボーンを非表示にする */
  ...
  private void computeWorldMatrices(
      ProcessedSkin skin, Matrix4f[] localMatrices, Matrix4f[] worldMatrices) {
    // ルートボーンから開始
    for (ProcessedBone rootBone : skin.rootBones()) {
      ...
      while (!stack.isEmpty()) {
        ...
        worldMatrices[current.index()].set(parentWorld).mul(currentLocal);
  ...
  private void renderVertex(
      ProcessedMesh mesh,
      int vertexIndex,
      Matrix4f @Nullable [] boneMatrices,
    ...
    float[] baseNormals =
        mesh.getNormals() != null ? mesh.getNormals().getFloatDataReadOnly() : null;
    ...
    // 即座に描画
    vertexConsumer
        .vertex(matrixStack.peek().getPositionMatrix(), x, y, z)
  ...
  /** 単一頂点へのモーフィング適用 */
  private Vector3f applyMorphingToVertex(
      ProcessedMesh mesh, int vertexIndex, float[] morphWeights, float x, float y, float z) {
    Vector3f result = new Vector3f(x, y, z);
    List<MorphTarget> morphTargets = mesh.morphTargets();

    for (int targetIndex = 0;
        targetIndex < morphTargets.size() && targetIndex < morphWeights.length;
        targetIndex++) {
   ...
   */
  private Vector3f applySkinning(
      ProcessedMesh mesh,
      int vertexIndex,
      Matrix4f[] boneMatrices,
      float x,
  ...
  private Vector3f applySkinningToVertex(
      ProcessedMesh mesh, int vertexIndex, Matrix4f[] boneMatrices, float x, float y, float z) {
    return applySkinning(mesh, vertexIndex, boneMatrices, x, y, z, true);
  }

  private Vector3f applySkinningToNormal(
      ProcessedMesh mesh, int vertexIndex, Matrix4f[] boneMatrices, float nx, float ny, float nz) {
