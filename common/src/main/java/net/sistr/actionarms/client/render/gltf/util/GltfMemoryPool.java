...
/** glTF描画用のThreadLocalメモリプール アロケーション削減とGC負荷軽減のための配列・オブジェクトプール */
public class GltfMemoryPool {
  ...
  public static PoolStats getStats() {
    return new PoolStats(
        FLOAT_POOL.get().getStats(), MATRIX_POOL.get().getStats(), INT_POOL.get().getStats());
  }
  ...
  public static class PoolStats {
    ...
    PoolStats(
        ArrayPoolStats floatArrayStats,
        ArrayPoolStats matrixArrayStats,
        ArrayPoolStats intArrayStats) {
  ...
  public static class ArrayPoolStats {
    ...
    public String toString() {
      return String.format(
          "%s: borrow=%d, return=%d, pooled=%d, hit_rate=%.2f%%",
