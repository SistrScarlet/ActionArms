...
public class GltfSkinExtractor {
  ...
  private String getNodeName(NodeModel node, int fallbackIndex) {
    ...
    return name != null && !name.trim().isEmpty() ? name.trim() : "Joint_" + fallbackIndex;
  }
  ...
  private void setInverseBindMatrices(ProcessedBone.Builder[] builders, SkinModel skin) {
    ...
    if (numMatrices != builders.length) {
      throw new RuntimeException(
          String.format(
              "Inverse bind matrix count (%d) doesn't match joint count (%d)",
  ...
  private ProcessedBone[] buildBoneHierarchy(
      ProcessedBone.Builder[] builders, List<NodeModel> jointNodes) {
