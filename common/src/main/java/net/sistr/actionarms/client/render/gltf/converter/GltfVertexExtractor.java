...
/** glTFメッシュからProcessedMeshへの変換を行うクラス AccessorDataCacheを使用してアクセサデータの重複を排除し、メモリ効率を向上 */
public class GltfVertexExtractor {
    ...
    public List<ProcessedMesh> extractMeshes(
            Map<MaterialModel, ProcessedMaterial> materials,
        ...
        for (int i = 0; i < primitives.size(); i++) {
            ...
            try {
                ...
                String meshName =
                        primitives.size() > 1 ? baseMeshName + "_Primitive_" + i : baseMeshName;

                ProcessedMesh processedMesh =
                        extractSinglePrimitive(materials, associatedSkin, primitive, meshName, i);

                processedMeshes.add(processedMesh);

            } catch (Exception e) {
                ActionArms.LOGGER.error(
                        "Failed to process primitive {} of mesh {}: {}",
                        i,
                        baseMeshName,
                        e.getMessage());
            }
        ...
        ActionArms.LOGGER.debug(
                "Cache stats after processing mesh {}: {}", baseMeshName, accessorCache.getStats());
    ...
    private String getMeshName(MeshModel meshModel) {
        ...
        return name != null && !name.trim().isEmpty()
                ? name.trim()
                : "Mesh_" + System.identityHashCode(meshModel);
    }

    private ProcessedMesh extractSinglePrimitive(
            Map<MaterialModel, ProcessedMaterial> materials,
            @Nullable ProcessedSkin associatedSkin,
        ...
        Map<String, AccessorData> attributeData =
                extractAttributeData(primitive, meshName, primitiveIndex);
    ...
    private Map<String, AccessorData> extractAttributeData(
        ...
        for (Map.Entry<String, AccessorModel> entry : attributes.entrySet()) {
            ...
            try {
                ...
                String accessorId =
                        String.format("%s_P%d_%s", meshName, primitiveIndex, attributeName);
                AccessorData data = accessorCache.getOrCreate(accessor, dataType, accessorId);

                attributeData.put(attributeName, data);

                ActionArms.LOGGER.debug(
                        "Extracted attribute {} for mesh {}: {}",
                        attributeName,
                        meshName,
                        data.getDebugInfo());
            } catch (Exception e) {
                ActionArms.LOGGER.error(
                        "Failed to extract attribute {} for mesh {}: {}",
                        attributeName,
                        meshName,
                        e.getMessage());
            }
    ...
    private void ensureRequiredAttributes(
        ...
        if (!attributeData.containsKey("NORMAL")) {
            ...
            AccessorData defaultNormals =
                    createDefaultNormals(vertexCount, meshName, primitiveIndex);
    ...
    /** デフォルト法線データを作成 */
    private AccessorData createDefaultNormals(
            int vertexCount, String meshName, int primitiveIndex) {
        ...
        return new AccessorData(
                id, AccessorDataType.NORMAL, vertexCount, normals, false, signature);
    }
    ...
    /** インデックスデータの抽出 */
    private AccessorData extractIndexData(
            MeshPrimitiveModel primitive, String meshName, int primitiveIndex) {
        ...
        try {
            ...
            return accessorCache.getOrCreate(
                    indexAccessor, AccessorDataType.SCALAR_INT, accessorId);
        } catch (Exception e) {
            ActionArms.LOGGER.error(
                    "Failed to extract indices for mesh {}: {}", meshName, e.getMessage());
            // フォールバック：連番インデックスを生成
            int vertexCount = getVertexCount(primitive);
            return createSequentialIndices(vertexCount, meshName, primitiveIndex);
        }
    ...
    /** 連番インデックスを作成 */
    private AccessorData createSequentialIndices(
            int vertexCount, String meshName, int primitiveIndex) {
        ...
        return new AccessorData(
                id, AccessorDataType.SCALAR_INT, vertexCount, indices, false, signature);
    }

    /** モーフターゲットの抽出（AccessorDataCache使用版） */
    private List<MorphTarget> extractMorphTargets(
            MeshPrimitiveModel primitive, int primitiveIndex) {
        ...
        for (int i = 0; i < targets.size(); i++) {
            ...
            try {
                ...
                if (positionDelta != null) {
                    ...
                    positionDeltas =
                            accessorCache.getOrCreate(
                                    positionDelta, AccessorDataType.MORPH_POSITION, deltaId);
                }
                ...
                if (normalDelta != null) {
                    ...
                    normalDeltas =
                            accessorCache.getOrCreate(
                                    normalDelta, AccessorDataType.MORPH_NORMAL, deltaId);
                }
                ...
                ActionArms.LOGGER.error(
                        "Failed to create morph target {}: {}", targetName, e.getMessage());
            }
        }
