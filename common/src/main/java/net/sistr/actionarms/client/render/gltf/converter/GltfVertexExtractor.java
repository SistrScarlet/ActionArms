...
public class GltfVertexExtractor {
  ...
  public List<ProcessedMesh> extractMeshes(
    ...
    for (int i = 0; i < primitives.size(); i++) {
      ...
      try {
        ...
        String meshName = primitives.size() > 1 ? baseMeshName + "_Primitive_" + i : baseMeshName;
        ...
        ActionArms.LOGGER.error(
            "Failed to process primitive {} of mesh {}: {}", i, baseMeshName, e.getMessage());
      }
    }
  ...
  private Map<String, AccessorData> extractAttributeData(
    ...
    for (Map.Entry<String, AccessorModel> entry : attributes.entrySet()) {
      ...
      try {
        ...
        ActionArms.LOGGER.debug(
            "Extracted attribute {} for mesh {}: {}", attributeName, meshName, data.getDebugInfo());
      } catch (Exception e) {
        ActionArms.LOGGER.error(
  ...
  private AccessorData createSequentialIndices(
    ...
    AccessorSignature signature =
        new AccessorSignature(0, 0, 5125, vertexCount, "SCALAR", false); // UNSIGNED_INT, SCALAR
    ...
    for (int i = 0; i < targets.size(); i++) {
      ...
      try {
        ...
        if (positionDelta != null) {
          ...
          positionDeltas =
              accessorCache.getOrCreate(positionDelta, AccessorDataType.MORPH_POSITION, deltaId);
        }
        ...
        if (normalDelta != null) {
          ...
          normalDeltas =
              accessorCache.getOrCreate(normalDelta, AccessorDataType.MORPH_NORMAL, deltaId);
        }
