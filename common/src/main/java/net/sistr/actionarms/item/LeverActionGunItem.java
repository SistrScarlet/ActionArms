...
public class LeverActionGunItem extends GunItem {
  ...
  public LeverActionGunItem(
      Settings settings, LeverActionGunData gunData, MagazineData magazineData) {
    super(settings);
  ...
  public void appendTooltip(
    ...
    if (gunComponent.getChamber().canShoot()) {
      tooltip.add(
          Text.translatable("item.actionarms.gun.chamber.loaded").formatted(Formatting.YELLOW));
    } else if (!gunComponent.getChamber().isEmpty()) {
      tooltip.add(
          Text.translatable("item.actionarms.gun.chamber.empty_caliber")
              .formatted(Formatting.YELLOW));
    } else {
      tooltip.add(
          Text.translatable("item.actionarms.gun.chamber.empty").formatted(Formatting.GRAY));
    }
    ...
    tooltip.add(
        Text.translatable("item.actionarms.gun.magazine.ammo", currentAmmo, maxAmmo)
  ...
  public float fireSpread(LivingEntity user, LeverActionGunComponent gunComponent) {
    ...
    // 移動状態による拡散増加
    boolean isMoving =
        user.getVelocity().horizontalLengthSquared() > 0.01 || (!user.isOnGround() && !isUserFly);
    if (isMoving) {
  ...
  /**
   * 拡散角を考慮した射撃方向ベクトルを計算する 真上・真下向きでも正しく拡散するように3D直交座標系を構築
   *
   ...
   */
  public static Vec3d calculateSpreadDirection(
      Vec3d baseDirection, float spreadAngleDegrees, Random random) {
    // 拡散角が0なら元の方向をそのまま返す
    ...
    Vec3d spreadDirection = forward.add(right.multiply(spreadX)).add(up.multiply(spreadY));

    return spreadDirection.normalize();
  }

  /** 前方ベクトルに垂直な右ベクトルを計算 真上・真下問題を解決するための特別処理 */
  ...
  public Reloadable.ReloadTickContext createReloadTickContext(
      Entity user, @Nullable Inventory inventory) {
    return new LeverActionReloadTickContext(user, inventory);
  }

  private record LeverActionReloadTickContext(Entity user, @Nullable Inventory inventory)
      implements Reloadable.ReloadTickContext {
